...
void load_terrain() {
  ...
  terrain.m = GenerateTerrain(&heightmap_texture);
  LoadTGATextureSimple("media/Terrain/maskros512.tga", &(terrain.t));
  terrain.t2 = 0;
//  terrain.pos = SetVector(-384,0,-384);
//  terrain.sca = SetVector(3,3,3);
  terrain.pos = SetVector(-128,0,-128);
  terrain.sca = SetVector(1,1,1);
  terrain.rot = SetVector(0,0,0);
  terrain.k_d = 1.0;
  terrain.shininess = 0.0;
  terrain.alpha = 1.0;
}


void draw_terrain() {
  draw_static_object(&terrain);
}
